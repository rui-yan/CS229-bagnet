...
optimizer_ft = optim.SGD(params_to_update, lr=INITIAL_LR, momentum=0.9)
# optim.Adam(model_ft.parameters(), lr=INITIAL_LR)
...
for i in range(start_epoch, TOTAL_EPOCH):
  ...
  if results['val_acc'] >= best_val_acc:
    ...
    torch.save(model_ft.state_dict(), './bagnet33/model_val%d.pt'%(best_val_acc*10000))
...
model_ft.load_state_dict(torch.load('./bagnet33/model_val%d.pt'%(best_val_acc*10000)))

print("==> Saving loss and accuracy data...")
out=open('./bagnet33/model_val%d.txt'%(best_val_acc*10000), 'w')
...
plt.savefig("./bagnet33/loss_acc_plot_bagnet33.png")
